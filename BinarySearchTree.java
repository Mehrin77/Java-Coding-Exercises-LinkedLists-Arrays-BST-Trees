...
public class BinarySearchTree<KeyType extends Comparable<KeyType>, ValueType> implements Map<KeyType, ValueType> {
	...
	private BSTNode remove(KeyType key, BSTNode root) {
	 ...
	 if (root.left ==null) {   //for subtree
		 return root.right; 
	 }
	 if (root.right==null) {
		 return root.left;
	 }
	 root.value= this.get (getMinKey (root.right), root.right) ; 
	 root.key = getMinKey(root.right);  //root.key is a node(2children) that we are replacing
	 root.right = remove(root.key,root.right); //min from right		 
